...
def start_ngrok_if_needed(local_host="127.0.0.1", port=5000, webhook_path="/webhook"):
    """
    依環境變數啟動 ngrok，回傳 public_url（字串或 None）
    .env 支援：
      - START_NGROK=1/0
      - NGROK=<完整路徑或放 PATH>
      - NGROK_REGION=jp/us/eu/ap 等
      - NGROK_ARGS=其他參數（例: "--inspect=true"）
    """
...
if not CHANNEL_SECRET or not CHANNEL_ACCESS_TOKEN:
    print("[HINT] 檢查：1) .env 與 app.py 同層、不是 .env.txt；2) 無空格/引號/Bearer；3) UTF-8；4) 你用的 conda 環境已安裝 python-dotenv")
...
if __name__ == "__main__":
    # 你的 webhook 路徑是 /webhook（上面 @app.route 已經寫好）
